...
void Oscyloskop(void)
{
	...
	while(!(keys==P_EXIT))
	{
		...
		if(autoset && keys ==0)
		{
			...
			avg = ((int32_t)avg*Gain1)/4864;
			if(v_max>40+avg || v_min<-40+avg)if(Vdiv1>0){Vdiv1--; autoset = 1;}
			if(v_max<15+avg && v_min+avg>-15)if(Vdiv1<6){Vdiv1++; autoset = 1;}
		...
		}else if(type==3)
		{
			...
			int32_t tmp;
			tmp = ((int8_t)cursor[1]-(int8_t)cursor[0])*((int32_t)pgm_read_word(&Time_tab[Sdiv]));
			if(Sdiv<5)
				tmp=tmp*10/8;
			else if(Sdiv<10)
				tmp=tmp/8;
			...
			if(keys == P_CURS + P_OK)
			{
				uint16_t* buf = (uint16_t*)kan1_in;
				LCDGoTo(0,7);
				LCDText_p(PSTR("Write..."));
				for(uint16_t i=0; i<512; i++)
					buf[i] = (kan1_lcd[cursor[0] + i*(int16_t)(cursor[1] - cursor[0])/512] - 96)*64;
					//buf[i] = (uint16_t)(kan1_lcd[i] - 128)*64;
				EepromTabWrite(e_kan_out, (uint8_t*)buf);
			}
		}
