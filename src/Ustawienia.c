...
#ifdef POLSKI
const uint8_t menu_ust_tab[6][12] PROGMEM=
{
	"Ustawienia ", //polski
	"Kalibracja ",
	"RS232 - PC ",
...
/**hektoPredkosci UART (Wojciech Meler)*/
...
{
	LCDGoTo(0,0);
	LCDText_p((const char*)menu_ust_tab[0]);
	for(uint8_t i=0; i<MENU_UST_TAB_I; i++)
	{
		LCDGoTo(10, i+1);
		if(i==menu)
			LCDTextNeg_p((const char*)menu_ust_tab[i+1]);
		else
			LCDText_p((const char*)menu_ust_tab[i+1]);
	}
}
...
void Kalibracja(void)
{
	LCDClearScreen();
	LCDGoTo(0,0);
	LCDText_p((const char*)menu_ust_tab[1]);
	uint8_t keys=0;
...
void RS232(void)
{
	LCDClearScreen();
	LCDGoTo(0,0);
	LCDText_p((const char*)menu_ust_tab[2]);
	uint8_t keys=0;
	...
	while(keys!=P_EXIT)
	{
		...
		LCDU16(pgm_read_word(&rsHectoSpeedTab[speed]));
		LCDText_p(PSTR("00"));
		if(keys == P_RIGHT)
			speed = SetRsSpeed(1);
		if(keys == P_LEFT)
...
void Wyswietlacz(void)
{
	LCDClearScreen();
	LCDGoTo(0,0);
	LCDText_p((const char*)menu_ust_tab[3]);
	uint8_t keys=0;
	...
	while(keys!=P_EXIT)
	{
		keys=Keyboard();
		LCDGoTo(0,1);
		LCDText_p((const char*)menu_ust_tab[4]);
		LCDU8(bright);
		LCDGoTo(0,2);
		LCDText_p((const char*)menu_ust_tab[5]);
		LCDU8(contrast);
		if(keys == P_UP)
			bright = LCDBright(1);
		if(keys == P_DOWN)
